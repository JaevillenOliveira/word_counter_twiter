	...
	while(ifstream_ob)
	{
      ...
      if(ch != ' ' and ch != '\t' and ch != '\n' and isascii(ch) and ch != EOF){ 
         ...
         if(isValidWord(str)){
            ...
            if(itSet == nonWords.end()){ // if the string belongs to the non-words list
...
int main(int argc, char** argv){
   ...
   if (dr) {
      int count = 0;
      // ########### parallel part start
      while ((en = readdir(dr)) != NULL and count <= numFiles) {
         cout << dirPath + '/' + en->d_name << endl;
         handleTweet(dirPath + '/' + en->d_name, words, nonWords); 
         count++;
      }
      closedir(dr); 
